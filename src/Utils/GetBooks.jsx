...
export default function GetBooks() {
    ...
    return (
        <div data-aos="fade-up" className="my-10 bg-gray-100 px-2 rounded-lg">
            ...
            >
                {books.map(book => (
                    <SwiperSlide key={book.id} onClick={() => navigateToMainBook(book.id)}>
                        <Link
                            
                            className="w-40 h-64 lg:h-80 lg:w-52 bg-white flex m-5 flex-col justify-center text-center items-center gap-y-4 rounded-lg shadow cursor-pointer p-5 overflow-hidden hover:scale-105 transition-all duration-300"
                        >
                            <img
                                src={book.img}
                                className='w-36 h-44 object-cover'
                                alt={book.name ? `Cover of ${book.name}` : 'Book cover'}
                                loading="lazy"
                            />
